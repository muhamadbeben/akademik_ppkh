...
class RaporPdfService {
  ...
  String _sanitizeText(String text) {
    return RaporUtils.sanitizeText(text);
  }
  ...
  String _formatNamaKelas(String kelasAsli) {
    ...
    return kelasAsli
        .replaceAll(RegExp(r'kelas\s*', caseSensitive: false), '')
        .trim();
  }

  Future<void> generateAndOpenRapor(
      RaporModel rapor, Uint8List? logoBytes) async {
    final pdf = pw.Document();
    final statusKenaikan = _cekStatusKenaikan(rapor.daftarNilai);

    // Filter nilai sesuai aturan baru
    final listAkademik = rapor.daftarNilai
        .where((n) => !_isKategoriHafalan(n.mataPelajaran))
        .toList();
    final listHafalan = rapor.daftarNilai
        .where((n) => _isKategoriHafalan(n.mataPelajaran))
        .toList();
    ...
    if (listAkademik.isEmpty) {
      tableAkademikData
          .add(['-', 'Belum ada data nilai akademik', '-', '-', '-']);
    ...
    if (listHafalan.isEmpty) {
      tableHafalanData
          .add(['-', 'Belum ada data nilai hafalan lisan', '-', '-', '-']);
    } else {
    ...
    // =========================================================================
    final nilaiSikapText =
        rapor.nilaiSikap > 0 ? rapor.nilaiSikap.toStringAsFixed(0) : '-';
    final predikatSikapText =
        rapor.predikatSikap.isNotEmpty ? rapor.predikatSikap : '-';
    final nilaiKehadiranText = rapor.nilaiKehadiran > 0
        ? rapor.nilaiKehadiran.toStringAsFixed(0)
        : '-';
    final predikatKehadiranText =
        rapor.predikatKehadiran.isNotEmpty ? rapor.predikatKehadiran : '-';
    final catatanAdabText = _sanitizeText(
      rapor.catatanAdab.isNotEmpty ? rapor.catatanAdab : '-',
    );
    ...
    final List<List<String>> tableSikapKehadiranData = [
      [
        ...
        'Nilai Kehadiran',
        nilaiKehadiranText,
        predikatKehadiranText,
        'Skor kehadiran santri'
      ],
      [
        '2',
        'Perilaku dan Sikap',
      ...
      [
        '3',
        'Kehadiran - Sakit',
        rapor.absenSakit.toString(),
        '-',
        '${rapor.absenSakit} Hari'
      ],
      [
        '4',
        'Kehadiran - Izin',
        rapor.absenIzin.toString(),
        '-',
        '${rapor.absenIzin} Hari'
      ],
      [
        '5',
        'Kehadiran - Tanpa Keterangan',
        rapor.absenAlpha.toString(),
        '-',
        '${rapor.absenAlpha} Hari'
      ],
    ];
            ...
            children: [
              ...
              pw.Row(
                ...
                children: [
                  ...
                  pw.Column(
                    ...
                    children: [
                      pw.Text('PONDOK PESANTREN KHOIRUL HUDA',
                          style: pw.TextStyle(
                              fontSize: 14, fontWeight: pw.FontWeight.bold)),
                      pw.SizedBox(height: 2),
                      pw.Text('YAYASAN PENDIDIKAN ISLAM NURUL IMAN',
                          style: pw.TextStyle(
              ...
              pw.TableHelper.fromTextArray(
                border: pw.TableBorder.all(
                    color: PdfColors.black, width: 1.2), // Hitam Tebal
                headerStyle:
              ...
              // =======================================================
              pw.Text('C. Penilaian Sikap & Kehadiran',
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold, fontSize: 10)),
              pw.SizedBox(height: 5),
              pw.TableHelper.fromTextArray(
  ...
  pw.Widget _infoText(String label, String val) => pw.Padding(
        ...
        child:
            pw.Text('$label : $val', style: const pw.TextStyle(fontSize: 10)),
      );
