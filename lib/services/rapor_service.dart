...
class RaporService {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Gagal cetak PDF: $e')));
      }
    }
  }
  ...
  static Future<RaporModel?> getRaporBySantri(
    ...
    try {
      ...
      if (query.docs.isNotEmpty) {
        final r =
            _parseFromRaporDoc(query.docs.first.data(), query.docs.first.id);
        if (r != null && r.daftarNilai.isNotEmpty) return r;
      }
  ...
  static RaporModel? _parseFromRaporDoc(Map<String, dynamic> d, String docId) {
    try {
      final rapor = RaporDataParser.parseRaporDoc(d, docId);
      return rapor.daftarNilai.isEmpty ? null : rapor;
    } catch (e) {
  ...
  static Future<RaporModel?> generateRapor(
    ...
    try {
      ...
      // Rata-rata berbobot
      final nilaiAkhir = (kehadiran * 0.05) +
          (perilaku * 0.05) +
          (avgUts * 0.20) +
          (avgUas * 0.40) +
          (hafalFinal * 0.30);
      ...
      return RaporModel(
        ...
        predikat: prediksiAi.isNotEmpty
            ...
            : RaporUtils.getPredikat(nilaiAkhir),
        catatanWaliKelas:
            'Tingkatkan terus semangat belajarmu, pertahankan prestasimu.',
